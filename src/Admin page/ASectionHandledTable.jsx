...
const columns = [
  ...
  { id: 'grade_level', label: 'Grade level', minWidth: 170, align: 'center'},
  { id: 'section_name', label: 'Section', minWidth: 170, align: 'center' },
  { id: 'name', label: 'Adviser', minWidth: 170, }, // Change to "teacher_name"
  {
    id: 'details',
    label: 'View details',
    minWidth: 170,
                    ...
                    <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                      {columns.map((column) => {
                        ...
                        return (
                          ...
                          >
                            {column.id === 'createdate' // Format the date
                              ? formatDateTime(row.created_at)
                              : column.id === 'details'
                              ? column.render(row, handleViewProfile) // Pass handleViewProfile as the onClick handler
                              : column.id === 'name'
                              ? `${row.teacher.firstName} ${row.teacher.middleName} ${row.teacher.lastName}`
                              : value}
                          </TableCell>
                        );
                      })}

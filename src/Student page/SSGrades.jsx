...
import SSGradeTable from "./SSGradeTable";
import axios from 'axios';
import { useUser } from '../UserContext';

export default function SSGrades({ onCancelClick }) {
  ...
  useEffect(() => {
    const fetchGrades = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8081/api/get_student_grades/${loggedInUser.user_id}/${selectedGradeLevel}`);
  ...
  }, [loggedInUser.user_id, selectedGradeLevel]);

  useEffect(() => {
    const fetchGradeLevels = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8081/api/get_student_grade_levels/${loggedInUser.user_id}`);
        if (response.data.success) {
